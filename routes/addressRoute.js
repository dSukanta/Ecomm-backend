    ...
    if (token !== token2) {
      ...
      if (result.user) {
        ...
        if (Exist) {
               ...
               res.status(200).send({status:200, message:'Address fetched successfully', data: addresses});
        } else {
  ...
  if (token && token2) {
    if (token !== token2) {
      ...
      if (result.user) {
        const Exist = await User.findOne({ email: result?.user });
        if (Exist) {
                const { name,phone_no,city,locality,district,state,pincode}= req.body;
                if(!name||!phone_no||!district||!state||!pincode){
                    res.status(400).send({status:400,message:'all fields mandatory'})
                }else{
                    const newAddress= new Address({name,phone_no,city,locality,district,state,pincode,user:Exist._id});
                    await newAddress.save();
                    res.status(201).send({status:201,message:`New Address added Successfully.`})
                }
        } else {
          res.status(500).send({ status: 500, message: `User not found` });
        }
...
addressRoute.put('/myaddress/:user/:addressid',async(req,res)=>{
    const token = req?.headers?.authorization?.split(" ")[1];
      ...
      if (result.user) {
        ...
        if (Exist) {
                ...
                if(!name||!phone_no||!district||!state||!pincode){
                    ...
                    const updateAddress= await Address.findByIdAndUpdate({_id: addressid},{...req.body});

                    if(updateAddress){
                        res.status(201).send({status:201,message:`Address updated Successfully.`})
                    }else{
                        res.status(500).send({status:500,message:`Internal server error`})
                    }
                }
